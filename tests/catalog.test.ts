...
test('content dates are stable across repository-wide commits', () => {
	const article = loadCatalog().articles.find(article => article.url.includes('AboutUs'))!
	assert.ok(article)
	assert.equal(article.updated, '2026-09-01')
	assert.equal(article.updatedTime, Date.parse('2026-09-01'))
})
